...
class JTNNVAE_joint(nn.Module):

    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG, property_weight, z_prop_size):
        super(JTNNVAE_joint, self).__init__()
        self.vocab = vocab
        ...
        self.z_prop_size = z_prop_size

        # Modified property predictor with LeakyReLU and layer normalization
        ...
        self.propNN = nn.Sequential(
                nn.Linear(self.z_prop_size, 32),
                nn.ReLU(),
                nn.Linear(32,16),
                nn.ReLU(),
                nn.Linear(16, 6) # first 3 means, last 3 stds
        )
        self.prop_loss = self.nll_gaussian_3prop
        self.prop_loss_weight = property_weight
    ...
    def nll_gaussian_3prop(self, y_pred, y_true):
        means = y_pred[:, :3]
        log_stds = y_pred[:, 3:]
        stds = torch.exp(log_stds)
        loss = ((means - y_true) ** 2) / (2 * stds ** 2) + log_stds
        return loss[:, 0].mean(), loss[:, 1].mean(), loss[:, 2].mean()

    def nll_gaussian_1prop(self, y_pred, y_true):
        means = y_pred[:, 0]
        log_stds = y_pred[:, 1]
        stds = torch.exp(log_stds)
        loss = ((means - y_true) ** 2) / (2 * stds ** 2) + log_stds
    ...
    def custom_loss(self, predictions, targets):
        ...
        for i in range(3):
            ...
            prop_losses.append(mse_loss + 0.1 * std_loss)
            mse_loss_total += mse_loss
            std_loss_total += std_loss
        return prop_losses
    ...
    def encode_from_smiles(self, smiles_list):
        ...
        tree_var = self.T_var(tree_vecs)
        mol_mean = self.G_mean(mol_vecs)
        mol_var = self.G_var(mol_vecs)
        
        z_tree, _ = self.rsample(tree_vecs, self.T_mean, self.T_var)
        z_mol, _ = self.rsample(mol_vecs, self.G_mean, self.G_var)
        
        return torch.cat([z_tree, z_mol], dim=-1)

    def predict_property_from_smiles(self, smiles_list):
        tree_batch = [MolTree(s) for s in smiles_list]
        _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
        tree_vecs, _, mol_vecs = self.encode(jtenc_holder, mpn_holder)
        ...
        z_prop = torch.cat([z_mol[:, :self.z_prop_size//2], z_tree[:, :self.z_prop_size//2]], dim=-1)

        property_predictions = self.propNN(z_prop)
    ...
    def forward(self, x_batch, beta):
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder, homos, lumos, r2s = x_batch
        x_tree_vecs, x_tree_mess, x_mol_vecs = self.encode(x_jtenc_holder, x_mpn_holder)
        z_tree_vecs,tree_kl = self.rsample(x_tree_vecs, self.T_mean, self.T_var)
        z_mol_vecs,mol_kl = self.rsample(x_mol_vecs, self.G_mean, self.G_var)

        z_prop = torch.cat([z_mol_vecs[:, :self.z_prop_size//2], z_tree_vecs[:, :self.z_prop_size//2]], dim=-1)

        # Convert lists to tensors if they're not already
        homos = homos.clone().detach().to(dtype=torch.float32, device=z_prop.device)
        lumos = lumos.clone().detach().to(dtype=torch.float32, device=z_prop.device)
        r2s = r2s.clone().detach().to(dtype=torch.float32, device=z_prop.device)

        # Stack the properties into a single tensor
        target = torch.stack([homos, lumos, r2s], dim=1)

        property_predictions = self.propNN(z_prop)
        prop_losses = self.prop_loss(property_predictions, target)  # Use MSE loss
        prop_loss = prop_losses[0] + prop_losses[1] + prop_losses[2]
        ...
        total_loss = word_loss + topo_loss + assm_loss + beta * kl_div + self.prop_loss_weight * prop_loss

        return total_loss, kl_div.item(), word_acc, topo_acc, assm_acc, prop_loss.item(), prop_losses[0], prop_losses[1], prop_losses[2]
