...
def main_vae_train(train,
    ...
    wandb.init(project="jtvae_train")
    ...
    for epoch in tqdm(range(epoch)):
        ...
        for batch in loader:
            ...
            if total_step % print_iter == 0:
                meters /= print_iter
                logging.info("[%d] Beta: %.3f, KL: %.2f, Word: %.2f, Topo: %.2f, Assm: %.2f, PNorm: %.2f, GNorm: %.2f", total_step, beta, meters[0], meters[1], meters[2], meters[3], param_norm(model), grad_norm(model))
                meters *= 0
            ...
            logging.info("Total step: %d, Loss: %.3f", total_step, loss.item())

            # Log loss and accuracy values to wandb
            wandb.log({
                "Loss": loss.item(),
                "KL Divergence": kl_div,
                "Word Accuracy": wacc * 100,
                "Topo Accuracy": tacc * 100,
                "Assembly Accuracy": sacc * 100,
