...
from property_prediction.train import PropertyPredictor
...
def analyze_molecule(vae_model, predictor, smiles, vocab, entire_data, n_encodings=10, n_decodings=10):
    ...
    try:
        ...
        # Lists to store all generated molecules
        all_generated = []
        all_predictions = []
        all_real_values = []
        ...
        tree_vecs, _, mol_vecs = vae_model.encode(jtenc_holder, mpn_holder)
        
        # Get initial latent vectors
        z_prop = torch.cat([mol_vecs[:, :], tree_vecs[:, :]], dim=-1)
        print("a")
        input_predictions = predictor(z_prop)
        print("b")
        ...
        for _ in range(n_encodings):
            tree_vecs, _, mol_vecs = vae_model.encode(jtenc_holder, mpn_holder)
            z_tree, _ = vae_model.rsample(tree_vecs, vae_model.T_mean, vae_model.T_var)
            z_mol, _ = vae_model.rsample(mol_vecs, vae_model.G_mean, vae_model.G_var)
            
            # Perform multiple decodings for each encoding
            for _ in range(n_decodings):
                try:
                    decoded_smiles = vae_model.decode(z_tree, z_mol, prob_decode=True)
                    if decoded_smiles and Chem.MolFromSmiles(decoded_smiles):
                        # Get predictions
                        tree_batch = [MolTree(decoded_smiles)]
                        _, new_jtenc_holder, new_mpn_holder = tensorize(tree_batch, vocab, assm=False)
                        new_tree_vecs, _, new_mol_vecs = vae_model.encode(new_jtenc_holder, new_mpn_holder)
                        
                        z_prop = torch.cat([new_mol_vecs[:, :], new_tree_vecs[:, :]], dim=-1)
                        pred = predictor(z_prop)
                        pred_norm = pred[:, :3].cpu().detach().numpy()[0]
...
def main(smiles, pp_path, model_path, vocab_path, data_path):
    try:

        # Paths to model and vocab files
        model_path = '/home/ece/Inverse-Design-For-Molecules/property_prediction/pp_model_seperately_trained/property_predictor.pth'
        vocab_path = '/home/ece/Inverse-Design-For-Molecules/data/vocab.txt'
        model_path_jtvae = '/home/ece/Inverse-Design-For-Molecules/fast_molvae/qm9_train_data/qm9_train_data/model.epoch-19'
        ...
        results = analyze_molecule(vae_model, predictor, smiles, vocab, entire_data)
        if results is None:
...
if __name__ == "__main__":
    smiles = "C[C@@H]1C=C[C@H](O)[C@H](C)O1"  
    vocab_path = '../../data/vocab.txt' 
    model_path = '/home/ece/Inverse-Design-For-Molecules/property_prediction/pp_model_seperately_trained/model.epoch-19'
    pp_path = "/home/ece/Inverse-Design-For-Molecules/property_prediction/pp_model_seperately_trained/property_predictor.pth"
    data_path = '../../data/qm9_smiles_prop_normalized.csv'
    
    regular_grid, safe_name = main(smiles, pp_path, model_path, vocab_path, data_path)
    if regular_grid is not None:
        regular_grid.save(f"separate/encoding_analysis_{safe_name}.png")
    else:
