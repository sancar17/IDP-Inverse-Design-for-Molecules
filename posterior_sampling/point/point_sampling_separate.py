...
def plot_direct_property_predictions(vae_model, predictor, samples, property_index, prop_mean, prop_std, target_value, property_name, save_path, dataset_samples):
    """Plot KDE of property predictions with dataset distribution"""
    with torch.no_grad():
        out = predictor(samples)
...
def plot_log_likelihood(trajectories, save_path):
    ...
    plt.ylim(-30, 1)  # Set reasonable y-axis limits
    plt.savefig(save_path)
    plt.close()
...
def decode_encode_posterior_samples(vae_model, predictor, posterior_samples, property_index, prop_mean, prop_std, target_value):
    ...
    with torch.no_grad():
        # Initial predictions
        out = predictor(posterior_samples)
        y_pred = out[:, property_index] * prop_std + prop_mean
        ...
        for i, z in enumerate(posterior_samples):
            for j in range(decode_times):
                ...
                smiles = vae_model.decode(z_tree, z_mol, prob_decode=False)
...
def process_property(vae_model, predictor, property_name, target_values, property_index, prop_mean, prop_std, analysis_dir, entire_data):
    ...
    for target_value in target_values:
        ...
        samples, mcmc, log_likelihoods = sample_posterior(
            predictor, target_value, property_index, prop_mean, prop_std
        )
        ...
        print("plot direct prediction")
        # Direct predictions
        direct_preds = plot_direct_property_predictions(
            vae_model, predictor, samples, property_index, prop_mean, prop_std,
        ...
        valid_mols, predictions, log_likelihoods = decode_encode_posterior_samples(
            vae_model, predictor, samples, property_index, prop_mean, prop_std, target_value
        )
    ...
    print("plot property kde direct")
    plot_property_kde(
    ...
    print("plot property kde selected")
    plot_property_kde(
        dataset_samples=dataset_samples,
...
def main():
    ...
    analysis_dir = './point_sampling_results_seperate_training/'
    ...
    predictor = PropertyPredictor(56,256)
    checkpoint = torch.load(pp_path)
